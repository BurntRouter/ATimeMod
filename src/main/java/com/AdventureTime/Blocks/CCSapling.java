...
public class CCSapling extends ATimeBlock {
	public CCSapling() {
		super(Material.rock);
		float f = 0.4f;
	...
	@Override
	public boolean canPlaceBlockAt(World w, int p_149742_2_, int p_149742_3_,
			int p_149742_4_) {
		return super.canPlaceBlockAt(w, p_149742_2_, p_149742_3_, p_149742_4_)
				&& this.canBlockStay(w, p_149742_2_, p_149742_3_, p_149742_4_);
	}

	@Override
	public boolean canBlockStay(World w, int i, int j, int k) {
		if (w.getBlock(i, j - 1, k) == Blocks.grass || w.getBlock(i, j - 1, k) == Blocks.dirt || w.getBlock(i, j - 1, k) == ModBlocks.CandiedDirt) {
			return true;
		}
		return false;
	}

	protected void checkAndDropBlock(World p_149855_1_, int p_149855_2_,
		...
		if (!this.canBlockStay(p_149855_1_, p_149855_2_, p_149855_3_,
			...
			this.dropBlockAsItem(p_149855_1_, p_149855_2_, p_149855_3_,
					p_149855_4_, p_149855_1_.getBlockMetadata(p_149855_2_,
							p_149855_3_, p_149855_4_), 0);
	...
	public void onNeighborBlockChange(World p_149695_1_, int p_149695_2_,
		...
		this.checkAndDropBlock(p_149695_1_, p_149695_2_, p_149695_3_,
				p_149695_4_);
	}

	@Override
	public boolean onBlockActivated(World w, int i, int j, int k,
			EntityPlayer player, int p_149727_6_, float p_149727_7_,
			float p_149727_8_, float p_149727_9_) {
		WorldGenCCTree tree = new WorldGenCCTree();
		if (!w.isRemote) {
			if (player.getCurrentEquippedItem() != null
					&& player.getCurrentEquippedItem().getItem() == Items.sugar) {
				if (!player.capabilities.isCreativeMode)
					--player.getCurrentEquippedItem().stackSize;
				Random r = new Random();
				int growChance = r.nextInt(5);
				if (growChance == 3)
					return tree.generate(w, r, i - 2, j, k - 2);
			}
		}
		return false;
	}
